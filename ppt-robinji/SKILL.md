...
| **输入源扩展** | **PDF / DOCX / Markdown / TXT 自动解析为大纲（v2.1+）** |
| **动画与过渡** | **29 种过渡效果 + 10 种形状动画（v2.2+）** |
| **原生形状** | **SVG → DrawingML + 自定义几何（v2.3+）** |
| **演讲音频** | **TTS 演讲旁白（Edge / OpenAI / 离线 / Mock）v2.4+** |
| **A11y 检查** | **WCAG AA/AAA + 色盲模拟 + JSON/MD/HTML 报告（v2.5+）** |
| **双语生成** | **中英双语 PPT（4 版式 + 术语表 + AI 翻译）v2.6+** |
...
```typescript
import { PPTCreator, AIGenerator } from 'ppt-robinji';
...
await creator.createFromOutline(content);
await creator.save('output.pptx');  // 自动应用动画
```

**支持的过渡效果（29 种）**：
fade / push / wipe / cover / cut / split / reveal / circle / diamond / plus / wedge / zoom / honeycomb / flash / vortex / ripple / glitter / newsflash / fall / drape / curtains / wind / prestige / fracture / crush / peeloff / pageturn / pan / random

**支持的形状动画（10 种）**：
fade / float / scale / slide / bounce / rotate / grow / shrink / appear / dissolve

**直接使用 AnimationManager**：
```typescript
import { AnimationManager } from 'ppt-robinji/pptx';
const mgr = new AnimationManager();
await mgr.applyAnimations('input.pptx', [
  { transition: { type: 'wipe', duration: 600 } },
  { transition: { type: 'split', duration: 800, direction: 'l' } },
]);
```

### 5.6 原生形状（v2.3+）

通过 SVG 或编程方式构建原生 DrawingML 形状：

```typescript
import { parseSVG, buildShapeXml, star, arrow, injectXML } from 'ppt-robinji/pptx';
